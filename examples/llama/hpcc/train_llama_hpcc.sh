...
if [ -z "$SCRATCH" ]; then
  echo "SCRATCH environment variable is not set. Please set SCRATCH to a valid path for checkpoints and data caching."
  exit 1
fi

CHECKPOINT_PATH=${1:-"$SCRATCH/checkpoints/llama3_8b_$DTYPE"}
TENSORBOARD_LOGS_PATH=${2:-"$SCRATCH/tensorboard_logs/llama3_8b_$DTYPE"}
