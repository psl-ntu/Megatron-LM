#!/bin/bash
# Training launcher — all config is injected via env vars from clusters/<name>/train.pbs.
...
PP_SIZE=${PP_SIZE:-1}
CP_SIZE=${CP_SIZE:-1}
VIRTUAL_PIPELINE_STAGES=${VIRTUAL_PIPELINE_STAGES:-0}
...
if [[ $ENABLE_FSDP -gt 0 ]]; then
    ...
    TRAINING_ARGS+=(--ckpt-format fsdp_dtensor)
else
    TRAINING_ARGS+=(--ckpt-format torch_dist)
fi
